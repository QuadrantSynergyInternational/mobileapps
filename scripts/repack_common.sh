...
#          BUNDLE_OUTPUT_DIR to be set. Optional: SENTRY_RELEASE, SENTRY_DIST.
common_upload_sentry() {
	if [[ -n "${SENTRY_AUTH_TOKEN:-}" && -n "${SENTRY_ORG:-}" && -n "${SENTRY_PROJECT:-}" ]]; then
		local sentry_release="${SENTRY_RELEASE:-$APP_VERSION}"
		local sentry_args=(
			--org "$SENTRY_ORG"
		...
		if [[ -n "${SENTRY_DIST:-}" ]]; then
			sentry_args+=(--dist "$SENTRY_DIST")
		fi

		echo ""
		echo "📡 Uploading sourcemaps to Sentry (release: $sentry_release${SENTRY_DIST:+, dist: $SENTRY_DIST})..."
