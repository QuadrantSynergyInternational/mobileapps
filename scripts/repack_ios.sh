...
echo "🔄 Bumping iOS version string to $APP_VERSION in Info.plist..."
/usr/libexec/PlistBuddy -c "Set :CFBundleShortVersionString $APP_VERSION" "$RELEASE_DOWNLOAD_DIR/unpacked_ipa/Payload/$APP_NAME/Info.plist"
...
if [[ -f "$BUNDLE_OUTPUT_DIR/app.manifest" ]]; then
  echo "📋 Injecting Expo Updates app.manifest into EXUpdates.bundle..."
  mkdir -p "$RELEASE_DOWNLOAD_DIR/unpacked_ipa/Payload/$APP_NAME/EXUpdates.bundle"
  cp "$BUNDLE_OUTPUT_DIR/app.manifest" "$RELEASE_DOWNLOAD_DIR/unpacked_ipa/Payload/$APP_NAME/EXUpdates.bundle/app.manifest"
fi
